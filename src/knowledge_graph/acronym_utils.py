...
Utilities for consuming cached acronym dictionaries produced by
managers.acronym_extractor.

This file does NOT extract acronyms from PDFs.

Its role in the knowledge graph pipeline is:
- load per-document acronym JSON caches;
- expose clean acronym maps for downstream graph steps;
- support deterministic entity validation by checking whether:
    1. an acronym short form appears in the section text;
    2. that acronym expands to the extracted concept long form;
- support safe expansion of raw LLM acronym-only outputs before KG writing.
