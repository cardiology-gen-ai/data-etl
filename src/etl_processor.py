...
class ETLProcessor(metaclass=Singleton):
        ...
        try:
            if self.index_manager.vectorstore.vectorstore_exists():
                ...
                self.index_manager.vectorstore.load_vectorstore(
                    embeddings_model=self.config.indexing.embeddings, retrieval_mode=self.config.indexing.retrieval_mode)
                ...
                self.index_manager.vectorstore.create_vectorstore(embeddings_model=self.config.indexing.embeddings)
    ...
    def process_file(self, filename: str, filepath: Optional[pathlib.Path] = None,
                     md_filepath: Optional[pathlib.Path] = None, force_md_conv: bool = True,
                     existing_metadata_path: str | pathlib.Path | None = None) -> Tuple[bool, DocumentMetadata | None]:
    ...
    def _save_docs_metadata(self, docs_metadata_list: List[DocumentMetadata]) -> None:
        ...
        """
        metadata_filename = "documents_metadata"
        if self.config.indexing.embeddings:
            embedding_name = self.config.indexing.embeddings.model_name.replace("/", "_")
            metadata_filename += f"_{embedding_name}"
    ...
    def perform_etl(self, force_md_conv: bool = True, existing_metadata_path: str | pathlib.Path | None = None) -> None:
        ...
        try:
            for filename in input_files:
                conversion_status, doc_metadata = self.process_file(
                    filename, force_md_conv=force_md_conv, existing_metadata_path=existing_metadata_path
                )
